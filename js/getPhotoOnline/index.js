const getPhotoJson = {
  ...
  renderBox(
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      imgsHtml =
        imgsHtml +
        `<div class="fancybox"><a href="${path}" class="fancybox" data-fancybox="images" data-caption="${title}"><img src="${path}" alt="${title}" /></a><span class="image-caption">${title}</span></div>`;
    }
  ...
  getJsonAPIForSites(cfg) {
    ...
    // }
    $(el).append(
      '<div class="loading"><i class="fas fa-cog fa-2x fa-spin"></i><p>正在加载</p></div>'
    );
    this.requestJsonAPI(
      cfg.api,
      function (data) {
        ...
        if (dt.length > 0) {
          ...
          $(el)
            .find(".waterfall-picture-container")
            .append(`<p style="color:red">${cfg.api} 没有图片链接</p>`);
        }
      },
      function () {
